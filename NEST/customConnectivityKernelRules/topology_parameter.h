...
/**
 * Random parameter with exponential distribution, optionally truncated to
 * [min,max). Truncation is implemented by rejection.
 */

class ExponentialParameter2 : public TopologyParameter
{
public:
  /**
   * Parameters:
   * tau - lambda of exponential distribution
   * d_min - minimum delay
   * d_max - maximum delay
   */
  ExponentialParameter2( const DictionaryDatum& d )
    : TopologyParameter( d )
    , tau_( 1.0 )
    , min_( 0.1 )
    , max_( 1.0 )
    , rdev()
  {
    ...
    if ( tau_ <= 0 )
      throw BadProperty(
        "topology::ExponentialParameter2: "
        "tau > 0 required." );
    if ( max_ < min_ )
      throw BadProperty(
        "topology::ExponentialParameter2: "
  ...
  //delay = ((c->DMin + (c->DMax - c->DMin) * log(1.0 - Random() * (1.0 - TailNeglected)) * InvLogTN) - DelayMin) / DelayStep;
  #define TailNeglected 0.05
  double
    raw_value( librandom::RngPtr& rng ) const
  {
    double val;
    ...
    do
    {
      //val = -std::log( rdev( rng ) ) / tau_;            //6 with lambdaD = [0.045,0.045,0.150]
      //val = -std::log( 1. - rdev( rng ) * 0.95 ) / tau_;   //5 with lambdaD = [0.045,0.045,0.150]
      //val =ceil((-std::log( 1. - rdev( rng ) * 0.95 ) / tau_)-min_);   //5 with lambdaD = [0.037,0.037,0.149]
      val = ((min_+ (max_ - min_) * std::log( 1. - rdev( rng ) * (1.0 - TailNeglected)) * InvLogTN) - min_) / 1.;   //7 no lambdaD needed
    } while ( ( val < min_ ) or (val > max_) or ( val!=val ));
    //val = -std::log( rdev( rng ) ) / tau_;              //3
    //val = -std::log( 1 - rdev( rng ) * 0.95 ) / tau_;   //4
    //val = tau_ * std::exp( -rdev( rng ) / tau_);
    //printf ("val = %f \n",val);
 ...
 double
  ...
  clone() const
  {
    return new ExponentialParameter2( *this );
  }

private:
  double tau_, min_, max_;
  //librandom::NormalRandomDev rdev;
  librandom::UniformRandomDev rdev;
};
...
/**
 * Exponential parameter3 with special rounding:
 * p(d) = c + a*exp(-d/tau).
 */
class ExponentialParameter3 : public RadialParameter
{
public:
  /**
   * Parameters:
   ...
   * c   - constant offset
   */
  ExponentialParameter3( const DictionaryDatum& d )
    : RadialParameter( d )
    , a_( 1.0 )
    , c_( 0.0 )
    , tau_( 1.0 )
  ...
  {
    double gamma;
    gamma = c_ + a_ * (round(std::exp( -x / tau_ )*1000)/1000);
    //printf("Kernel = %f for x = %f \n",gamma,x);
    return gamma;
  }
  ...
  clone() const
  {
    return new ExponentialParameter3( *this );
  }
