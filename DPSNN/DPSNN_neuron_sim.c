...
void neuronClass::causalSTDP_ms_ofBackwardSynapses(const int32_t localSpikeTime_ms) {
  uint32_t synOffset;
...
void neuronClass::regularDynamicTimeStep(
  const int32_t thisTimeStep_ms, const float *pInputDistrib, uint32_t * pThalInputCounter) {
  ...
  DPSNNverboseStart(false,thisTimeStep_ms,lnp_par.debugPrintEnable_ms);
  if (glob_n==168) 
    printf("START of current funnel on neu=%d at %d ms \n",glob_n,thisTimeStep_ms);
  DPSNNverboseEnd();
...
#undef reproducibleCode
  ...
  if ((glob_n % lnp_par.tabNeuron) == 0){
    ...
    DPSNNverboseStart(false,thisTimeStep_ms,lnp_par.debugPrintEnable_ms);
      printf("TabSeed: on H=%d at ms=%d neu=%d timeStepModule=%d thisTab=%d seed=%d \n",
 ...
 // local step of LIFCA neuron dynamic
  ...
  for(i=0;i<inputCurrentsCount;i++){
	...
	for(i=0;i<inputCurrentsCount;i++)
	  printf("Real Input Current nosort: at %d ms, on neu %d: inputCurrent.value=%f inputCurrent.time=%f \n",
  ...
  if (glob_n==168)
    if(inputCurrentsCount){
      for(i=0;i<inputCurrentsCount;i++)
	printf("Real Input Current sorted: at %d ms, on neu %d: inputCurrent.value=%f inputCurrent.time=%f \n",
	       thisTimeStep_ms,glob_n,inputCurrentArray[i].value,inputCurrentArray[i].time);}
  DPSNNverboseEnd(); 
...
if (invNuExt>0){
  if (assignedExternalSpikeTime < nextTimeStepF) {
	...
	DPSNNverboseStart(false,thisTimeStep_ms,lnp_par.debugPrintEnable_ms); 
	if (glob_n==168)
	  printf("Bath Input Current: at %d ms, on neu %d: stimulusCurrent.value=%f stimulusCurrent.time=%f \n",
	...
	DPSNNverboseStart(true,thisTimeStep_ms,lnp_par.debugPrintEnable_ms); 
	printf("WARNING: on h=%d at ms %d on neuron %d the number of generated bath currents exceeded the maximum number allowed, fixed to the value maxExternCurrentCount=%d. Enlarge this value to allow more external currents.\n",lnp_par.loc_h,thisTimeStep_ms,glob_n,maxExternCurrentCount);
	DPSNNverboseEnd();
      }
	...
	printf("Total input currents: at %d ms, on neu %d: inputCurrent.value=%f inputCurrent.time=%f \n",
	       thisTimeStep_ms,glob_n,inputCurrentArray[i].value,inputCurrentArray[i].time);
    DPSNNverboseEnd();  
  ...
  DPSNNverboseStart(false,thisTimeStep_ms,lnp_par.debugPrintEnable_ms);
  if (glob_n==168) 
    printf("END of current funnel and neu dynamic on neu=%d at %d ms \n",glob_n,thisTimeStep_ms);
  DPSNNverboseEnd();
...
void neuronClass::LIFCADynamic(const uint32_t thisTimeStep_ms, const double currentTime, const double thisInputCurrent) {
 ...
 // local step of LIFCA neuron dynamic
  DPSNNverboseStart(false,thisTimeStep_ms,lnp_par.debugPrintEnable_ms); 
  if (glob_n==2424)
    printf("DYN START neu=%d, v=%f, c=%f, I=%f at %d ms with currentTime=%f \n",
  ...
  if(((currentTime - (double)thisTimeStep_ms )>= 1.0 ) || ((currentTime - (double)thisTimeStep_ms)< 0 )) {
    printf("ERROR in neu dynamic: input currentTime in wrong ms: currentTime=%f, thisTimeStep_ms=%d on neu %d\n",
  ...
  DPSNNverboseStart(false,thisTimeStep_ms,lnp_par.debugPrintEnable_ms); 
  if (glob_n==2424)
    printf("LIFCA DYN: thisTimeStep_ms=%d currentTime=%f deltaT=%f TFLES=%f\n",
  ...
  if (TFLES > Tarp) {   //Updates the neuron state outside absolute refractory period (ARP)
    ...
    #define fatigueModulation
    #ifdef fatigueModulation
    {
       float modulated_gC;

       if(pLnp_dynPar==NULL) {
	 printf("ERROR: in neuron_sim(), pLnp_dynPar==NULL\n");
	 fflush(stdout); exit(0);
      };

      DPSNNverboseStart(false,1,0);
        if((lnp_par.loc_h==0) && (glob_n==0)) {
	  printf("HAVE WE ENTERED HERE?\n");
	  fflush(stdout);
	  printf("IF YES, ARE WE ABLE TO ACCESS THE  pLnp_dynPar->fatigueDynamic.additionalFatigue=%f?\n",
		 pLnp_dynPar->fatigueDynamic.additionalFatigue);
	  fflush(stdout);
	}
      DPSNNverboseEnd();
 
       if(gC!=0.0) {
	 modulated_gC=gC + pLnp_dynPar->fatigueDynamic.additionalFatigue;
       }else{
	 modulated_gC=gC;
       }
       ...
       DPSNNverboseStart(false,1,0);
       if((lnp_par.loc_h==0) && (glob_n==0)) {
	 printf("additionalFatigueAmplitude=%f Transition_ms=%f additionalFatigue=%f modulated_gC = %f at thisTimeStep_ms=%d\n",
       ...
       v = v - v * rmHalf - modulated_gC * c * deltaTHalf;
       v = v - v * rmHalf - modulated_gC * c * deltaTHalf;   
    };
    #else
...
#define inputCurrentModulation
#ifdef inputCurrentModulation
   {
       double modulatedInputCurrent;
       if(thisInputCurrent >=0.0) {	 
          modulatedInputCurrent =
	     thisInputCurrent *
	     pLnp_dynPar->inputCurrentDynamic.multiplierExcCurrent;
       }else{
	  modulatedInputCurrent =
	     thisInputCurrent *
	     pLnp_dynPar->inputCurrentDynamic.multiplierInhCurrent;
       };
    v += modulatedInputCurrent;
   }
#else    
    v += thisInputCurrent;
#endif
#undef inputCurrentModulation
  ...
  DPSNNverboseStart(false,thisTimeStep_ms,lnp_par.debugPrintEnable_ms);
  if (glob_n==2424)
    printf("DYN END neu=%d, v=%f, c=%f, I=%f at %d ms\n",
...
void neuronClass::regularDynamicTimeStep(
  ...
  DPSNNverboseStart(false,thisTimeStep_ms,lnp_par.debugPrintEnable_ms);  
  printf("DYN START neu=%d, v=%f, u=%f, I=%f at %d ms\n",
  ...
  DPSNNverboseStart(false,thisTimeStep_ms,lnp_par.debugPrintEnable_ms);  
  printf("DYN END neu=%d, v=%f, u=%f, I=%f at %d ms\n",
	 glob_n, v, u, InputCurrent,thisTimeStep_ms);
  DPSNNverboseEnd();   
