...
struct DPSNN_parameters getParameters(const uint32_t loc_h_initValue) {
  ...
  char *ps_env_startModulations_ms;
  char *ps_env_stopModulations_ms;
  ...
  struct DPSNN_parameters lnp_par;
   ...
   //start time for Modulations
   ps_env_startModulations_ms = getenv("env_startModulations_ms");
   if(ps_env_startModulations_ms!=NULL) {
      lnp_par.startModulations_ms = atoi(ps_env_startModulations_ms);
   }else{
     printf("ERROR: missing env_startModulations_ms param in DPSNN_script mpirun invocation\n");
     fflush(stdout);exit(0);
   }

   //stop time for Modulations
   ps_env_stopModulations_ms = getenv("env_stopModulations_ms");
   if(ps_env_stopModulations_ms!=NULL) {
      lnp_par.stopModulations_ms = atoi(ps_env_stopModulations_ms);
   }else{
     printf("ERROR: missing env_stopModulations_ms param in DPSNN_script mpirun invocation\n");
     fflush(stdout);exit(0);
   }

   DPSNNverboseStart(true,1,0);
   if(lnp_par.loc_h==0) {
       printf("getParameters():         startModulations_ms=%d\n",
	      lnp_par.startModulations_ms);
       printf("getParameters():         stopModulations_ms=%d\n",
	      lnp_par.stopModulations_ms);
   };
   DPSNNverboseEnd();
  ...
  // env_startRatesPerPop_s 
   ...
   readModulationsParametersFile(&lnp_par);
...
void readModulationsParametersFile(struct DPSNN_parameters *pLnp_par) {
  #define maxNumOfModulationsParam 20
  #define bufferSize 200
  ...
  uint32_t numOfTokenInLine;
  uint32_t numOfNonRemarkLines;
  float initValue[maxNumOfModulationsParam];
  uint32_t y,j;
   
  sprintf(fileName,"modulations.txt");
  fp = fopen(fileName,"r");
  if (fp == NULL) {
    if(pLnp_par->loc_h==0) {
      printf("WARNING: modulations.txt missing, -setting default modulations params \n");
      fflush(stdout);
    };
    pLnp_par->modulations.additionalFatigueAmplitude = 0.0;
    pLnp_par->modulations.additionalFatigueTransition_ms = 10000.0;
    pLnp_par->modulations.additionalFatigueStationary_ms = 10000.0;
    pLnp_par->modulations.multiplierInhCurrentExtreme = 1.0;
    pLnp_par->modulations.multiplierInhCurrentTransition_ms = 10000.0;
    pLnp_par->modulations.multiplierInhCurrentStationary_ms = 10000.0;
    pLnp_par->modulations.multiplierExcCurrentExtreme = 1.0;
    pLnp_par->modulations.multiplierExcCurrentTransition_ms = 10000.0;
    pLnp_par->modulations.multiplierExcCurrentStationary_ms = 10000.0;
    return;
  }
  else
  {
    for(j=1;j<maxNumOfModulationsParam;j++)
      initValue[j] = 0.0;
    ...
    if((numOfTokenInLine==0)||(numOfTokenInLine > maxNumOfModulationsParam)) {
      if(pLnp_par->loc_h==0) {
        printf("ERROR: modulations.txt %d columns (empty or too much params)\n",
	       numOfTokenInLine);
        fflush(stdout); exit(0);
      }
    }
	
    {
      ...
      pLnp_par->modulations.additionalFatigueAmplitude =
	               initValue[additionalFatigueAmplitudeEnum];
      counter++;	  
      pLnp_par->modulations.additionalFatigueTransition_ms =
	               initValue[additionalFatigueTransition_msEnum];
      counter++;
      pLnp_par->modulations.additionalFatigueStationary_ms =
	               initValue[additionalFatigueStationary_msEnum];
      counter++;

      pLnp_par->modulations.multiplierInhCurrentExtreme =
	               initValue[multiplierInhCurrentExtremeEnum];
      counter++;	  
      pLnp_par->modulations.multiplierInhCurrentTransition_ms =
	               initValue[multiplierInhCurrentTransition_msEnum];
      counter++;
      pLnp_par->modulations.multiplierInhCurrentStationary_ms =
	               initValue[multiplierInhCurrentStationary_msEnum];
      counter++;

      pLnp_par->modulations.multiplierExcCurrentExtreme =
	               initValue[multiplierExcCurrentExtremeEnum];
      counter++;	  
      pLnp_par->modulations.multiplierExcCurrentTransition_ms =
	               initValue[multiplierExcCurrentTransition_msEnum];
      counter++;
      pLnp_par->modulations.multiplierExcCurrentStationary_ms =
	               initValue[multiplierExcCurrentStationary_msEnum];
      counter++;



      //-1 because the first column is for algo	  
      if(counter != (numOfModulationsParamsEnum)) {
	if(pLnp_par->loc_h==0) {
	  printf("ERROR missing MODULATIONS params: initialized %d, expected %d\n",
		 counter,numOfModulationsParamsEnum);
	  fflush(stdout); exit(0);
	}
      }
    }
    ...
    DPSNNverboseStart(true,1,0);
      if(pLnp_par->loc_h==0) {
        printf("MODULATIONS PARAMS from file modulations.txt:\n");
	    	
        printf("modulations.additionalFatigueAmplitude = %f\n",
            pLnp_par->modulations.additionalFatigueAmplitude);
        printf("modulations.additionalFatigueTransition_ms = %f\n",
            pLnp_par->modulations.additionalFatigueTransition_ms);
        printf("modulations.additionalFatigueStationary_ms = %f\n",
            pLnp_par->modulations.additionalFatigueStationary_ms);

	printf("modulations.multiplierInhCurrentExtreme = %f\n",
            pLnp_par->modulations.multiplierInhCurrentExtreme);
        printf("modulations.multiplierInhCurrentTransition_ms = %f\n",
            pLnp_par->modulations.multiplierInhCurrentTransition_ms);
        printf("modulations.multiplierInhCurrentStationary_ms = %f\n",
            pLnp_par->modulations.multiplierInhCurrentStationary_ms);

	printf("modulations.multiplierExcCurrentExtreme = %f\n",
            pLnp_par->modulations.multiplierExcCurrentExtreme);
        printf("modulations.multiplierExcCurrentTransition_ms = %f\n",
            pLnp_par->modulations.multiplierExcCurrentTransition_ms);
        printf("modulations.multiplierExcCurrentStationary_ms = %f\n",
            pLnp_par->modulations.multiplierExcCurrentStationary_ms);
  ...
  #undef maxNumOfModulationsParam
  #undef bufferSize
}
