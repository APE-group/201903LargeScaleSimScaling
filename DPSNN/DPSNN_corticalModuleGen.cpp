...
void localNetClass::simpleCM_invokeIndivSynGenerators()
{
  ...
  for(i=0;i < lnp_par.locN; i++) {
    ...
    //after the following statement each neuron has a pointer to access (but not modify)
    //dynamic values set during the simulation by localNet
    n[i].initPointerToGlobalDynamicParametersAwareness(&lnp_dyn_par);
    n[i].initStat(pStat);
