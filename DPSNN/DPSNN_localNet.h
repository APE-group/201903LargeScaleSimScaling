...
private:
  ...
  //those parameters of the simulation maintained by localNet
  //dynamic are those changed during the simulations at localNet level
  struct DPSNN_parameters lnp_par;
  struct DPSNN_dynamicParameters lnp_dynPar;
...
uint32_t probableInterMod_nonRand_targetNeu_globId(
  ...
  void modulations_ms();
  void fatigueModulation_ms();
  void inhInputCurrentModulation_ms();
  void excInputCurrentModulation_ms(); 
};
